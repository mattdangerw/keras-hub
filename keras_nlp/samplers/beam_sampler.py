    ...
    def __call__(
        ...
        def flatten_beams(x):
            ...
            if isinstance(x, tf.Tensor):
                ...
                shape = ops.shape(x)
            flat_shape = (batch_size * self.num_beams,) + shape[2:]
        ...
        def unflatten_beams(x):
            ...
            if isinstance(x, tf.Tensor):
                ...
                shape = ops.shape(x)
            unflat_shape = (batch_size, self.num_beams) + shape[1:]
        ...
        log_probs = ops.array([[0.0] + [-1e9] * (self.num_beams - 1)])
        ...
        def body(prompt, cache, index, log_probs):
            ...
            next_log_probs = (
                ops.cast(ops.log(probs), log_probs.dtype) + log_probs[..., None]
            )
            ...
            def gather_beams(x):
                ...
                x = ops.take_along_axis(
                    x, ops.expand_dims(beam_indices, -1), axis=1
                )
        ...
        if self.return_all_beams:
            sorted_indices = ops.argsort(-1 * all_log_probs, axis=-1)
            sorted_indices = ops.cast(sorted_indices, "int32")
            sorted_log_probs = ops.take_along_axis(
                all_log_probs,
                sorted_indices,
                axis=-1,
            )
            sorted_indices = ops.expand_dims(sorted_indices, -1)
            ...
            top_beams = ops.argmax(all_log_probs, axis=-1)[:, None]
            top_beams = ops.cast(top_beams, "int32")
            top_beams = ops.expand_dims(top_beams, -1)
