...
def add_lora_layers(
    ...
    for weight in model.weights:
        # Update trainable weights to match `trainable_weights`.
        path = weight.path
        if any(re.match(pattern, path) for pattern in trainable_weight_paths):
    ...
    for layer in model._flatten_layers():
        ...
        if not any(re.match(pattern, path) for pattern in lora_layer_paths):
            continue
